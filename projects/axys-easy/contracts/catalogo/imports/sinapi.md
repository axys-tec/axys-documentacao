...
| `SINAPI_Manutenções` | Changelog mensal entre edições (criação/desativação/alteração), insumo e composição → `catalogo.sinapi_manutencoes` (**SINAPI-Diff**). Reconciliado com o **Axys-DIFF** (série histórica computada pela Axys edição-a-edição) — ver [estagios.md §A.7](estagios.md). |
...
4. **Composições** — CSD/CCD/CSE (custo de referência) + Analítico (estrutura) → `composicoes`, `composicoes_itens`, `composicoes_custo`. **Conferência** calculado×fonte (regras: listagem §2.2) — **gate de convergência** que valida o modelo SE-only (esperado: SINAPI converge ao centavo).
...
Regra canônica em [../listagem.md §2.1](../listagem.md). Resumo SINAPI:
...
- **EXCEÇÃO — MDO NUNCA pode ficar NC (2026-07-12).** A premissa antiga ("não há MO órfã") caiu: funções
  ...
  (+`aviso` de auditoria). Ver [VINCULACOES §3.1](../vinculacoes.md).
...
- **Três modalidades** em `composicoes_custo`: `SD` (CSD), `CD` (CCD) e **`SE`** (CSE = PELADO, sem encargos). O `SE` é gravado p/ consulta (telas leem direto, não recomputam) e o calculado do SE (Σ pelado×coef, **sem LS**, trunc) confere contra a CSE — fechando a doutrina SE-only no nível de composição. Ver [estagios.md §A.4](estagios.md).
- **Fidelidade canônica:** grava-se TODOS os itens do Analítico, inclusive **coef 0** (`CHECK ci_coef >= 0`) — a fonte publica coef 0 em CPU incompleta; não se julga, repete-se. Insumo SEM PREÇO coef-0 propaga SEM CUSTO (ex.: 104871). Ver [../listagem.md §2.2](../listagem.md).
...
> **Correção 2026-06-04:** a premissa "%AS = SP plano" estava errada — ignorava o coef da UF e subcontava MO (−1% a −4% em UFs com buraco). O atribuído é preço → **truncado a 2 casas**. Prova e detalhe em §7.
...
> ⚠️ **REVISTO 2026-07-13 (repense doc/path — `as capabilities (README) §11.9/§11.10`).** A vigência
...
- **Cadernos técnicos**: são da **FONTE** (como os livros), **NÃO por edição** (2026-07-11) —
  ...
  Ver [STORAGE_LAYOUT](../publicacao.md) §2. CDHU não usa (parse rápido, 1 doc, sem download).
...
livros). Sem mudança de constraint. Ver `schemas/schema.sql` e `as capabilities (README) §10/§11`.
