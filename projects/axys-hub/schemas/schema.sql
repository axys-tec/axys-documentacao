...
-- ============================================================
-- TABELA: hub_login_log
-- Eventos de autenticação (LOGIN / LOGOUT / LOGIN_FALHA) e a
-- ORIGEM/canal do login. Espelha a semântica de audit.login_logs
-- do AxysEasy, no padrão hub_*.
--
-- origem: canal real do login — distinto por rastreabilidade.
--   'LOCAL'   login direto no Hub (form próprio)
--   'SSO'     login federado entre apps do ecossistema (ADR-021)
--   'GOV_BR' | 'APPLE' | 'GOOGLE'  OAuth/IdP externos (futuro)
--   'API_KEY' autenticação programática
--
-- user_id / tenant_id nullable: LOGIN_FALHA pode não ter
-- usuário/tenant resolvido. FK ON DELETE SET NULL para o
-- registro de auditoria sobreviver à remoção do usuário/tenant.
-- (hub_audit_log usa CASCADE no tenant; aqui priorizamos a
--  imutabilidade do log de acesso.)
-- ============================================================
CREATE TABLE IF NOT EXISTS hub_login_log (
    login_id     UUID        NOT NULL DEFAULT gen_random_uuid(),
    login_no     BIGINT      NOT NULL GENERATED BY DEFAULT AS IDENTITY,
    user_id      UUID,
    tenant_id    UUID,
    email        TEXT,
    acao         TEXT        NOT NULL,
    ...
    CONSTRAINT hub_login_log_pkey
        PRIMARY KEY (login_id),

    CONSTRAINT hub_login_log_user_id_fkey
        FOREIGN KEY (user_id)
        REFERENCES hub_user (user_id)
        ON DELETE SET NULL,

    CONSTRAINT hub_login_log_tenant_id_fkey
        FOREIGN KEY (tenant_id)
        REFERENCES hub_tenant (tenant_id)
        ON DELETE SET NULL,

    CONSTRAINT ck_hub_login_log_acao
        CHECK (acao IN ('LOGIN', 'LOGOUT', 'LOGIN_FALHA')),

    CONSTRAINT ck_hub_login_log_origem
        CHECK (origem IN (
            'LOCAL',
            'SSO',
            'GOV_BR',
...
CREATE INDEX idx_hub_login_log_no
    ON hub_login_log (login_no);

CREATE INDEX idx_hub_login_log_user
    ON hub_login_log (user_id, created_at DESC)
    WHERE user_id IS NOT NULL;

CREATE INDEX idx_hub_login_log_email
    ON hub_login_log (email);
