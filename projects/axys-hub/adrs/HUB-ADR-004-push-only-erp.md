# ADR-027 — Arquitetura Push-Only (ERP → AxysHub → AxysGestorDB)
**Status:** Proposto (para aceite)  
**Data:** 2026-02-06  
**Área:** AxysHub / AxysGestor / Microapps ERP

---

## 1) Contexto
O AxysGestor é um dashboard de bolso. Muitos ERPs serão **locais** (ex.: LunalôSys), sem endpoint público e sem disponibilidade garantida para *pull* do Hub.
...
## 2) Decisão
A arquitetura oficial do AxysGestor é **Push-Only**:

ERP/microapp do cliente → envia snapshots/updates → AxysHub  
AxysHub valida e persiste em AxysGestorDB (cache) → iOS lê
...
- AxysGestorDB pode ser descartado sem impacto em CORE.
...
- O contrato do AxysGestor deve explicitar que o iOS é **read-only** e que o ingest é **separado**.
...
## 7) Registro
Esta ADR define o padrão de integração para o AxysGestor.
